...
Factor: neighborhood, neighborhood cleased, zipcode, property type, room type
...
listings$review_scores_rating <- ifelse(listings$review_scores_rating < avg_rating, "below", "above")  # above / below avg flag
listings <- listings[order(listings$review_scores_rating), ]  # sort
property_type_factor

property_type_levels
# Diverging Barcharts
ggplot(listings, aes(x=property_type_levels, y=review_scores_rating, label= 'Review Score')) + 
  geom_bar(stat='identity', aes(fill=review_scores_rating), width=.5)  +
  scale_fill_manual(name="Review Score", 
                    labels = c("Above Average", "Below Average"), 
                    values = c("above"="#00ba38", "below"="#f8766d")) + 
  labs(subtitle="Review Ratings", 
...
factors <- c(0, 50, 100, 150, 200, 250, 300, 350, 400)
listings$cleaning.cat <- cut(listings$cleaning_fee, breaks = factors)
cleaning_fee_factors <- factor(listings$cleaning_fee, levels = factors)
ggplot(listings) + aes(x = beds, y = cleaning_fee_factors) + geom_point()
```
